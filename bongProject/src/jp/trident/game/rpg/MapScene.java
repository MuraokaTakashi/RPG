...
public class MapScene implements IScene {
	...
	public void initialize() {
		...
		// ファイルを読み込む
		final int[][] mapData = gu.fileLoad(gu.context, "fieldMapData/sampleMap.map");
		final int[][] mapCollData = gu.fileLoad(gu.context, "fieldMapData/sampleColl.map");

		// マップを読み込む
		this.map.load(mapData);
		this.map.collLoad(mapCollData);
