...
public class Map {
	 ...
	 * マップのロード
	 */
	public void load(int[][] mapData) {
		final int w = this.baseMapObject.width;
		final int h = this.baseMapObject.height;
		final int sw = this.baseMapObject.ani.sw;
		final int sh = this.baseMapObject.ani.sh;
		final Bitmap img = this.baseMapObject.img;
		int _mapXLen = mapData[0].length;
		int _mapYLen = mapData.length;

		// 配列確保
		this.mapArray  = new DrawObject[_mapYLen][_mapXLen];
		this.chipArray = new int[_mapYLen][_mapXLen];

		int m = 0;
		int posX = 0;
		int posY = 0;
		for(int y = 0; y < _mapYLen; y++) {

			for(int x = 0; x < _mapXLen; x++) {
				m = mapData[y][x];
				posX = w * x;
				posY = h * y;
				DrawObject drawObj = new DrawObject(w, h);
				drawObj.imgLoad(img, sw, sh);
				drawObj.ani.loopEnable();
	 ...
	 * マップのコリジョンロード
	 */
	public void collLoad(int[][] collData) {
		int _mapXLen = collData[0].length;
		int _mapYLen = collData.length;

		this.collArray = new int[_mapYLen][_mapXLen];

		for(int y = 0; y < _mapYLen; y++) {
			for(int x = 0; x < _mapXLen; x++) {
				this.collArray[y][x] = collData[y][x];
			}
		}
	}
