...
public class TitleScene implements IScene {
	...
	public TitleScene() {
		...
		this.bg1.imgLoad(gu.context, R.drawable.bg, gu.bitmapfOption, 1024, 512);
		this.bg2.imgLoad(gu.context, R.drawable.bg, gu.bitmapfOption, 1024, 512);
		...
		int h = 470 / _w;
		int len = h * w;
		this.bg3 = new DrawObject[len];
		for(int i = 0; i < len; i++) {
			this.bg3[i] = new DrawObject(w, h);
			this.bg3[i].imgLoad(this.touch.img, 10, 10);
			this.bg3[i].ani.loopEnable();
			this.bg3[i].posX = (i % _w) * this.bg3[i].width;
			this.bg3[i].posY = (i / _w) * this.bg3[i].height;
		}
	...
	public void update() {
		...
		if(VirtualController.isTouchTrigger(0) == true) {
			...
			if(coll == true) {
				...
				gu.sceneManager.sceneChange(scene);
			}
...
//		for(int i = 0; i < this.bg3.length; i++) {
//			DrawObject dobj = this.bg3[i];
//			sv.ScaleDrawImage(dobj.img,
//					dobj.posX, dobj.posY,
//					dobj.ani.sx, dobj.ani.sy, dobj.ani.sw, dobj.ani.sh,
//					dobj.scaleX, dobj.scaleY, false);
//		}
